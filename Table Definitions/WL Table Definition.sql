CREATE sequence IF NOT exists wl_recno_seq;


CREATE TABLE IF NOT EXISTS public.wl
(
    recno bigint NOT NULL DEFAULT nextval('wl_recno_seq'::regclass),
    entity_id integer,
    ...
    CONSTRAINT wl_pkey PRIMARY KEY (recno),
    CONSTRAINT "Duplicate_WL" UNIQUE (entity_id, task_id, complete_date)
)

TABLESPACE pg_default;

ALTER TABLE IF EXISTS public.wl
    OWNER to postgres;

-- Trigger: update_edit_timestamp

-- DROP TRIGGER IF EXISTS update_edit_timestamp ON public.wl;
